...
run = Run(mode='online')
run.init(folder="/FNO_Camera", tags=['FNO', 'Camera', 'rbb', 'Forecasting', 'shot-agnostic', 'discretisation-invariant', 'Full Length', "Error Train"], metadata=configuration)
...
pred_set_error = torch.zeros(test_u.shape) #Prediction of the Error
test_u_error = torch.zeros(test_u.shape) #Error from previous
index = 0
...
test_l2 = (test_u_error - pred_set_error).pow(2).mean()
