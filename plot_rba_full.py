...
@author: vgopakum
FNO modelled over Camera data = rba camera looking at the central solenoid
"""
# %%
...
import os 
path = os.getcwd()
data_loc = '/Users/Vicky/Documents/UKAEA/Data/Camera_Data/rba_30250_30431'
...
# model_name = 'dyadic-parsley'# 250 16x32
# model_name = 'forgiving-range'# 500 8x16 step=20 
model_name = 'grizzled-novel' # 500 8x16



if device == torch.device('cpu'):
    checkpoint = torch.load(model_loc + '/FNO_rba_' + model_name + '.pth', map_location=torch.device('cpu')) #First 250. 
else:
    checkpoint = torch.load(model_loc + '/FNO_rba_' + model_name + '.pth') #First 250. 
...
test_shot_lens = [89, 177, 188]
shot_idx = np.random.randint(0,3) #Selecting a random shot from the test range. 
shot_idx = 1
...
pcm =ax.imshow(u_field[:,:,0], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_1, vmax=v_max_1)
...
pcm = ax.imshow(u_field[:,:,int(step/2)], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_2, vmax=v_max_2)
...
pcm = ax.imshow(u_field[:,:,-1], cmap='plasma',  extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_3, vmax=v_max_3)
...
# plt.savefig(file_loc + '/rba_feedforward10_'+model_name+'.pdf')
...
pcm =ax.imshow(u_field[:,:,0], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_1, vmax=v_max_1, aspect=aspect_ratio)
# ax.title.set_text('Initial')
ax.title.set_text('t='+ str(round(1.2*(time_idx+0), 1) + 18)+'ms')
ax.set_ylabel('Solution')
divider = make_axes_locatable(ax)
...
pcm = ax.imshow(u_field[:,:,int(step/2)], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_2, vmax=v_max_2, aspect=aspect_ratio)
# ax.title.set_text('Middle')
ax.title.set_text('t='+ str(round(1.2*(time_idx+5), 1) + 18)+'ms')
ax.axes.xaxis.set_ticks([])
ax.axes.yaxis.set_ticks([])
...
pcm = ax.imshow(u_field[:,:,-1], cmap='plasma',  extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_3, vmax=v_max_3, aspect=aspect_ratio)
# ax.title.set_text('Final'
ax.title.set_text('t='+ str(round(1.2*(time_idx+10), 1) + 18)+'ms')
ax.axes.xaxis.set_ticks([])
ax.axes.yaxis.set_ticks([])
...
pcm = ax.imshow(u_field[:,:,0], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], aspect=aspect_ratio)
...
pcm = ax.imshow(u_field[:,:,int(step/2)], cmap='plasma',  extent=[-1.0, 2.0, 0.0, 1.0], aspect=aspect_ratio)
ax.axes.xaxis.set_ticks([])
ax.axes.yaxis.set_ticks([])
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.1)
...
pcm = ax.imshow(u_field[:,:,-1], cmap='plasma',  extent=[-1.0, 2.0, 0.0, 1.0], aspect=aspect_ratio)
ax.axes.xaxis.set_ticks([])
ax.axes.yaxis.set_ticks([])
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.1)
...
plt.savefig(file_loc + '/rba_feedforward10_'+model_name+'_error.pdf')
...
ax = fig.add_subplot(3,3,1)
pcm =ax.imshow(u_field[0,:,:,-1], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_1, vmax=v_max_1, aspect=aspect_ratio)
# ax.title.set_text('Initial')
...
ax = fig.add_subplot(3,3,2)
pcm = ax.imshow(u_field[int(len(u_field)/2),:,:,-1], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_2, vmax=v_max_2, aspect=aspect_ratio)
# ax.title.set_text('Middle')
...
ax = fig.add_subplot(3,3,3)
pcm = ax.imshow(u_field[-1, :,:,-1], cmap='plasma',  extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_3, vmax=v_max_3, aspect=aspect_ratio)
# ax.title.set_text('Final')
ax.title.set_text('t='+ str((len(u_field)+  T_in + 5 + step )*1.2))
ax.title.set_text('t='+ str(round((len(u_field)+  T_in + 5 + step )*1.2, 1))+'ms')
...
ax = fig.add_subplot(3,3,7)
pcm = ax.imshow(u_field[0,:,:,-1], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], aspect=aspect_ratio)
ax.set_ylabel('Abs Error')
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.1)
cbar = fig.colorbar(pcm, cax=cax)
...
ax = fig.add_subplot(3,3,8)
pcm = ax.imshow(u_field[int(len(u_field)/2),:,:,-1], cmap='plasma',  extent=[-1.0, 2.0, 0.0, 1.0], aspect=aspect_ratio)
ax.axes.xaxis.set_ticks([])
ax.axes.yaxis.set_ticks([])
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.1)
...
ax = fig.add_subplot(3,3,9)
pcm = ax.imshow(u_field[-1,:,:,-1], cmap='plasma',  extent=[-1.0, 2.0, 0.0, 1.0], aspect=aspect_ratio)
ax.axes.xaxis.set_ticks([])
ax.axes.yaxis.set_ticks([])
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.1)
...
plt.savefig(file_loc + '/rba_last10_'+model_name+'_error.pdf')
...
# %% 
# create_video(camera_vals, 'rba_'+ str(test_shots[shot_idx]))
...
# %%
#############################################################################
#Creating the two kinds of plots that Stan wants for the paper: 
...
ax = fig.add_subplot(2,3,1)
pcm =ax.imshow(u_field[0,:,:,-1], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_1, vmax=v_max_1)
# ax.title.set_text('Initial')
...
ax = fig.add_subplot(2,3,2)
pcm = ax.imshow(u_field[int(len(u_field)/2),:,:,-1], cmap='plasma', extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_2, vmax=v_max_2)
# ax.title.set_text('Middle')
...
ax = fig.add_subplot(2,3,3)
pcm = ax.imshow(u_field[-1, :,:,-1], cmap='plasma',  extent=[-1.0, 2.0, 0.0, 1.0], vmin=v_min_3, vmax=v_max_3)
# ax.title.set_text('Final')
ax.title.set_text('t='+ str(round((len(u_field)+  T_in + 5 + step )*1.2, 1)))
ax.axes.xaxis.set_ticks([])
ax.axes.yaxis.set_ticks([])
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.1)
...
# plt.savefig(file_loc + '/rba_last10_'+model_name+'.pdf')

# %%
#Testing out for different resolutions. 


def time_windowing_flip(shots):
    u1 = [] #input a
    u2 = [] #output u
    data_loc = '/Users/Vicky/Documents/UKAEA/Data/Camera_Data/DifferentResolutions_RBA'
    for ii in shots:
        data = h5py.File(data_loc + '/'+'rba'+str(ii)+'.h5', 'r')
        data_length = int((len(data.keys()) - 3)/2)
        temp_cam_data = []
        for jj in range(data_length):
            if jj<10:
                temp_cam_data.append(np.fliplr(np.asarray(data['frame000'+str(jj)])))
            if jj>=10 and jj < 100:
                temp_cam_data.append(np.fliplr(np.asarray(data['frame00' + str(jj)])))
            if jj >= 100:
                temp_cam_data.append(np.fliplr(np.asarray(data['frame0' + str(jj)])))
...
test_shots = [29550, 29552]
test_a, test_u = time_windowing_flip(test_shots)
test_a = normalizer.encode(test_a)
test_u_encoded = normalizer.encode(test_u)
ntest = len(test_a)
...
test_shot_lens = [124, 119]
shot_idx = np.random.randint(0,2) #Selecting a random shot from the test range. 
# shot_idx = 1
time_idx =  np.random.randint(0,test_shot_lens[shot_idx])
# time_idx = 10
idx = int(np.sum(test_shot_lens[:shot_idx]) + time_idx )
